    ...
    static loadFromProject (project) {
      ...
      return BbPromise.mapSeries(models.walkSyncDirs(), (filename) => {
        ...
        .then(function (models) {
          ...
          // build main models object
          for (let i = 0; i < models.length; i++) {
            for (let model in models[i]) {
              if (_.has(models[i], model) && !_.startsWith(model, '_')) {
                if (model in modelsData ) throw new SError('Detected duplicate models names for model: ' + model + ' .Please use different model names');
                modelsData[model] = models[i][model];
              }
            }
          }
          
          return BbPromise.resolve(modelsData);
        });
    }
